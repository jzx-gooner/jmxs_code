...
def setCameraStatus():
    print("== 设置相机状态 ==")
    # ImmutableMultiDict([('', '{"items":[{"id":"c202da68-a4d5-4e08-a031-8708b36e806c"}，{"id":"c202da68-a4d5-4e08-a3232-8708b36e806c"}]}')])>
    print(request.form.get())
...
def addCamera():
    ...
    print(request.form.get)
    content = request.form['items[content]'] 
    id = request.form['items[id]'] 
    ext = request.form['items[ext]'] 
    user = request.form['items[user]'] 
    password = request.form['items[password]'] 
    ip = request.form['items[ip]'] 
    port = request.form['items[port]'] 
    options = request.form['items[options]'] 
    result = True
    if not result:
